...
__global__ void sharedMatrixMultiplication(double A[M][P], double B[P][N], double C[M][N]) {
...
int main() {
	...
	averageTime /= count;
	printf("[GPU - Double] (Matrix Multiplication 2D - Shared) Average Elapsed Time = %f ms\n", averageTime);
	return 0;
}
