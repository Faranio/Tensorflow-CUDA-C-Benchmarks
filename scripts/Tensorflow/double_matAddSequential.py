...
def matrix_add(verbose=False, device="/gpu:0"):
	...
	A = tf.ones([M, M], dtype=dtype) * 100000000
	B = tf.ones([M, M], dtype=dtype) * 200000000
...
def main():
	...
	averageTime /= count
	print("[CPU - Double] (Matrix Add) Average Elapsed Time = {} ms".format(averageTime))
